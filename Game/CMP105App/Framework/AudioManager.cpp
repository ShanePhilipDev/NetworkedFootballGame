...
AudioManager::AudioManager()
{
	...
	// adding all sounds and music to the manager
	addMusic("sfx/Waka_Waka.ogg", "music");
	addSound("sfx/Goal_Scored.ogg", "goal");
	addSound("sfx/Ball_Kicked.ogg", "kick");
	addSound("sfx/Post_Hit.ogg", "post");
}
...
// Goes through each sound object and changes the volume to the number provided.
void AudioManager::setSoundVolume(int v)
{
	for (int i = 0; i < sounds.size(); i++)
	{
		sounds[i].sound.setVolume(v);
	}
}
